...
local function create_worktree(path, branch, upstream, found_branch)
  ...
  local set_branch_args =
    { "branch", string.format("--set-upstream-to=%s/%s", upstream, branch) }
...
M.pick_worktree_path = function(call_on_path)
  local get_worktrees = function()
    local worktree_list = vim.api.nvim_cmd({
      cmd = "!",
    ...
    local worktrees = {}
    for wt in string.gmatch(worktree_list, "\n%S+%s+%S+%s+%S+") do
      -- Strip the worktree from '\n'
      wt = string.sub(wt, 2)

      -- Switch first and last item
      local worktree_table = {}
      local i = 1
      for word in string.gmatch(wt, "%S+") do
        if i == 1 then
          worktree_table.path = word
        elseif i == 2 then
          worktree_table.commit = word
        else
          worktree_table.branch = word
        end

        i = i + 1
      end
      worktrees[#worktrees + 1] = worktree_table
    end

    return worktrees
  end

  pickers.select(get_worktrees(), {
    prompt = "Worktrees: ",
    format_item = function(item)
      local show = item.branch .. " " .. item.commit .. " " .. item.path
      return show
    end,
  }, function(choice)
    -- If no choice, do nothing
    if choice == {} or not choice then
      return
    end

    -- Grab the last item of the choice, which is the path
    call_on_path(choice.path)
  end)
end

M.switch_worktree_picker = function()
  M.pick_worktree_path(M.switch_worktree)
end

M.delete_worktree_picker = function()
  ...
  M.pick_worktree_path(function(path)
    M.delete_worktree(path, force_next_deletion, {
